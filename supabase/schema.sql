...
create table public.employees (id uuid primary key default gen_random_uuid(), tenant_id uuid references public.tenants(id) on delete cascade, employee_code text not null, name text not null, department text, pin text not null, role_label text default 'employee', can_manage boolean default false, has_shift boolean default true, monthly_salary numeric(10,2) default 0, schedule jsonb default '{}', is_mixed boolean default false, daily_hours numeric(4,2), status text default 'active', created_at timestamptz default now(), unique(tenant_id, employee_code));
...
-- NUEVO — Planificador semanal de empleados mixtos
create table public.shift_plans (id uuid primary key default gen_random_uuid(), tenant_id uuid references public.tenants(id) on delete cascade, branch_id uuid, employee_id uuid references public.employees(id) on delete cascade, date_str date not null, entry_time_str text not null, duration_hours numeric(4,2) not null, exit_time_str text, notes text, created_by uuid, created_at timestamptz default now(), unique(tenant_id, employee_id, date_str));
...
create policy "shift_plans_select" on public.shift_plans for select using (tenant_id = public.my_tenant_id());
create policy "shift_plans_insert" on public.shift_plans for insert with check (tenant_id = public.my_tenant_id());
create policy "shift_plans_update" on public.shift_plans for update using (tenant_id = public.my_tenant_id());
create policy "shift_plans_delete" on public.shift_plans for delete using (tenant_id = public.my_tenant_id());
